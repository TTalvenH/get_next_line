...
int	vec_init_or_grow(t_vec *vec, size_t new_size)
{
	...
	else
	{
		...
		if (!tmp)
			return (-1);
		tmp[new_size] = '\0';
		ft_strlcpy(tmp, vec->str, vec->u + 1);
		free(vec->str);
		vec->str = NULL;
		vec->str = tmp;
	}
...
char	*vec_cut(t_vec *vec)
{
	...
	rtn = ft_strndup(vec->str, vec->i + 1);
	tmp = malloc (sizeof(char) * (vec->u - vec->i));
	if (!tmp)
		return (NULL);
	tmp[vec->u - (vec->i + 1)] = '\0';
	ft_strlcpy(tmp, &vec->str[vec->i + 1], vec->u - vec->i + 1);
	free(vec->str);
	vec->str = NULL;
	vec->u = vec->u - (vec->i + 1);
	vec->i = 0;
...
char	*copy(char *buff, int r)
{
	...
	if (r)
	{
		if (vec_init_or_grow(&vec, vec.u + r) < 0)
			return (NULL);
		vec.u = vec.u + r;
		ft_strlcat(vec.str, buff, vec.u + 1, vec.u - r);
	}
	while (vec.i < vec.u)
	{
